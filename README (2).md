...
| Name | Role |
|---|---|
| Mashrufud | Circuit Diagrams & Schematics |
| Taha | Tinkercad Simulation |
| Jisan | Physical Wiring & Hardware Assembly |
| Aynul | Arduino Code Development |

### What each of us did so far

**Mashrufud** drew the full circuit diagrams showing how every component connects — motors, sensors, motor driver, and power lines. This became the reference that the rest of the team worked from.

**Taha** built the virtual simulation in Tinkercad based on Mashrufud's diagrams. We used this to verify our wiring logic and pin assignments before touching any real hardware.
...
**Aynul** started writing the Arduino code — motor control functions, sensor reading logic, and the serial monitor output so we can watch what the robot is sensing in real time.
